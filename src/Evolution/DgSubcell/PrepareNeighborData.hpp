 ...
 * expense of projection and slicing. If any neighbors are doing FD, we
 * project and slice to all neighbors. If our own DG mesh's basis doesn't
 * support subcell (e.g. a non-hypercube topology like a filled cylinder or
 * ball), we always send unprojected DG volume data instead, since projecting
 * to a nonexistent subcell mesh is not possible.
