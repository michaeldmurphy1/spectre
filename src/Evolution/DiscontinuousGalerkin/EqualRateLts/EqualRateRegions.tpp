...
EqualRateRegions<Dim, tmpl::list<RegionGenerators...>,
  ...
  tmpl::for_each<tmpl::range<size_t, 0, sizeof...(RegionGenerators)>>(
      [&]<size_t N>(tmpl::type_<tmpl::size_t<N>> /*meta*/) {
        // Name the generator type outside the inner lambda: naming a data
        // member from a lambda with no capture, even in an unevaluated
        // operand, trips up some compilers.
        using generator_type = tmpl::at_c<tmpl::list<RegionGenerators...>, N>;
        std::get<N>(generators_) = std::apply(
            [](const auto&... gen_args) { return generator_type(gen_args...); },
