 ...
 *
 * Prefer this overload when the caller already has a `Domain`, since building
 * one is not cheap. Pass `domain.blocks()` for \p blocks, and the
 * `DomainCreator`'s `block_names()` and `block_groups()` for the name lookup.
 *
