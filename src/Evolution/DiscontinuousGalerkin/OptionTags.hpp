...
///
/// \note This lives outside the `DgSubcell` library (even though it is
/// primarily a DG-subcell concept) so that it can also be read by code that
/// must not depend on `DgSubcell`, such as the element distribution (see
/// `evolution::dg::Tags::OnlyDgBlockIds`). As a result it is a sibling of
/// `Subcell` rather than nested inside it.
