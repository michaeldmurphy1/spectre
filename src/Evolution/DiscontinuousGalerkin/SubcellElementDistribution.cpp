  ...
  for (size_t block_id = 0; block_id < initial_extents.size(); ++block_id) {
    if (not alg::found(only_dg_block_ids, block_id)) {
      ...
      for (size_t d = 0; d < Dim; ++d) {
        // Not an ASSERT: `subcell_extent_from_dg_extent(0)` underflows, which
        // would silently produce an enormous element cost in release builds.
        if (gsl::at(weighting_extents, d) == 0) {
          ERROR("The initial extents of block "
                << block_id << " in dimension " << d
                << " must be non-zero, but are zero.");
        }
        gsl::at(weighting_extents, d) =
            subcell_extent_from_dg_extent(gsl::at(weighting_extents, d));
      }
