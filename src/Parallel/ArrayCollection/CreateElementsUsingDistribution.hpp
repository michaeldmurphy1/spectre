...
void create_elements_using_distribution(
  ...
  if (element_weight.has_value()) {
    // Only allocate a modified copy of `initial_extents` if an override was
    // actually supplied, so the (common) no-override case pays no extra cost.
    ...
    // `BlockZCurveProcDistribution` only uses the extents to sanity check
    // their size, so pass the true `initial_extents` rather than the
    // weighting ones. The costs it distributes already account for the
    // override.
