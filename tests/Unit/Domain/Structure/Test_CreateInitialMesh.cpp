...
SPECTRE_TEST_CASE("Unit.Domain.Structure.CreateInitialMesh", "[Domain][Unit]") {
  ...
  {
    INFO("cartoon_fourier");
    const Element<3> cartoon_fourier(element_id_3d, {},
    ...
    for (const auto& i1_basis :
      ...
      for (const auto& i1_quadrature :
           {Spectral::Quadrature::GaussLobatto, Spectral::Quadrature::Gauss}) {
        CHECK(
            create_initial_mesh({{{3, 5, 1}}}, cartoon_fourier, i1_basis,
                                i1_quadrature) ==
            Mesh<3>{{{3, 5, 1}},
  ...
  CHECK_THROWS_WITH(
      create_initial_mesh(
          {{{3, 5, 1}}},
          Block<3>{nullptr, 0, {}, "", domain::topologies::cartoon_fourier},
          ElementId<3>{0,
