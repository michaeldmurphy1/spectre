...
namespace {
// A domain creator with a mix of subcell-capable (hypercube) and
// subcell-incapable (spherical shell / filled sphere) blocks, used to test
// auto-detection of blocks whose topology does not support subcell.
...
void test_block_groups() {
  ...
  CHECK(evolution::dg::compute_only_dg_block_ids(
            ...
            .size() == 1);
  // "Wedges" is a group of four blocks
  CHECK(evolution::dg::compute_only_dg_block_ids(
