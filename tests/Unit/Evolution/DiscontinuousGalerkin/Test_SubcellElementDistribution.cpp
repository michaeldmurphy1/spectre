...
namespace {
void test_subcell_extent_from_dg_extent() {
  static_assert(evolution::dg::subcell_extent_from_dg_extent(1) == 1);
  static_assert(evolution::dg::subcell_extent_from_dg_extent(4) == 7);
  static_assert(evolution::dg::subcell_extent_from_dg_extent(8) == 15);
}
...
void test_compute_weighting_extents_override() {
  ...
  // Zero extents would underflow, so they must be caught in release builds
  // too, not just by an ASSERT.
  CHECK_THROWS_WITH(
...
SPECTRE_TEST_CASE("Unit.Evolution.DG.SubcellElementDistribution",
                  "[Unit][Evolution]") {
  test_subcell_extent_from_dg_extent();
