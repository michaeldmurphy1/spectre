...
void test_non_conforming_interface_logical_coords(
  ...
  static_assert(VolumeDim == 3,
                "test_non_conforming_interface_logical_coords is only "
                "meaningful for a 3D domain.");
    ...
    for (const auto& [dir, nbrs] : block.neighbors()) {
      ...
      if (nbrs.ids().size() > 1) {
        for (const size_t neighbor_id : nbrs.ids()) {
          ...
          const auto xi_nbr = interface_logical_coordinates(face_mesh, nbr_dir);
      ...
      for (size_t pt = 0; pt < n_face_pts; ++pt) {
        ...
        const bool covered = alg::any_of(nbrs.ids(), [&](const size_t
                                                             neighbor_id) {
          return blocks[neighbor_id].stationary_map().inverse(x_pt).has_value();
        });
        CHECK(covered);
      }
    }
...
// Only meaningful for a 3D domain, so it is instantiated only there. The 1D
// and 2D instantiations would have had a body that never returns, which GCC
// flags under -Wsuggest-attribute=noreturn.
